...
async def delete_role(post_id: int = Path(description="岗位唯一ID"),
                      crud_async_session: AsyncGenericCRUD = Depends(crud_getter(SysPostModel)),
...
@post_router.put("/sys_role_change_status", summary="修改岗位状态")
async def sys_user_reset(change_status_param: ChangeStatusParam, crud_async_session: AsyncGenericCRUD = Depends(crud_getter(SysPostModel)),
...
async def get_dept_post_list_by_dept_id(dept_post_query_param: DeptPostQueryParam, crud_async_session: AsyncGenericCRUD = Depends(crud_getter(SysPostModel))):
